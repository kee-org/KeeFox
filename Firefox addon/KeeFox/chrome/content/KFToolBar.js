...
KFToolbar.prototype = {
    ...
    setOneLoginsMenu: function (container, group) {
        ...
        for (var i = 0; i < foundLogins.length; i++) {
            ...
            tempButton.addEventListener("command", function (event) { keefox_org.ILM.loadAndAutoSubmit(0, event.ctrlKey, usernameName, usernameValue, login.uRLs[0], null, null, login.uniqueID); event.stopPropagation(); }, false); //ael: works
            ...
            tempButton.addEventListener("click", function (event) { if (event.button == 1) { keefox_org.ILM.loadAndAutoSubmit(event.button, event.ctrlKey, usernameName, usernameValue, login.uRLs[0], null, null, login.uniqueID); event.stopPropagation(); if (event.button == 1) keefox_org.UI.closeMenus(event.target); } }, false); //ael: works
